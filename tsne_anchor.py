...
for img_path in img_list:
    ...
    if len(labels)<=1:
        tbox_gt[0] = labels[0] - labels[2] / 2  # top left x
        tbox_gt[1] = labels[1] - labels[3] / 2  # top left y
        tbox_gt[2] = labels[0] + labels[2] / 2  # bottom right x
        tbox_gt[3] = labels[1] + labels[3] / 2
    else:
        tbox_gt = xywh2xyxy(labels[:, 1:5]*1280)
    ...
    for i in range(len(tbox_gt)):
        ...
        for j in range(len(indices)):
            s_features_17.append(features[17][..., indices[j][0], indices[j][1]])
    ...
    for i in range(len(tbox_gt)):
        ...
        for j in range(len(indices)):
            s_features_20.append(features[20][..., indices[j][0], indices[j][1]])
    ...
    for i in range(len(tbox_gt)):
        ...
        for j in range(len(indices)):
            s_features_23.append(features[23][..., indices[j][0], indices[j][1]])
    ...
    print(len(np_features_17))
    print(count)

model = TSNE(n_components=2)
tsne = model.fit_transform(np_features_17)
# s_labels_name = ['person', 'bus', 'car', 'truck', 'bike']
# fig = plt.figure()
# xs = tsne[:, 0]
# ys = tsne[:, 1]
# scatter = plt.scatter(xs, ys, c=s_labels)
# handles, _ = scatter.legend_elements(prop='colors')
# plt.legend(handles, s_labels_name)
# plt.savefig('./runs/tsne/'+img_path)
...
for i in range(len(s_labels_17)):
    if s_labels_17[i]==0.0:
        xy_person_17.append(tsne[i])
    if s_labels_17[i]==1.0:
        xy_bus_17.append(tsne[i])
    if s_labels_17[i]==2.0:
        xy_car_17.append(tsne[i])
    if s_labels_17[i]==3.0:
        xy_truck_17.append(tsne[i])
    if s_labels_17[i]==4.0:
        xy_bike_17.append(tsne[i])

    count += 1

plt.scatter(np.array(xy_person_17)[:,0], np.array(xy_person_17)[:,1], label='person')
plt.scatter(np.array(xy_bus_17)[:,0], np.array(xy_bus_17)[:,1], label='bus')
plt.scatter(np.array(xy_car_17)[:,0], np.array(xy_car_17)[:,1], label='car')
plt.scatter(np.array(xy_truck_17)[:,0], np.array(xy_truck_17)[:,1], label='truck')
plt.scatter(np.array(xy_bike_17)[:,0], np.array(xy_bike_17)[:,1], label='bike')
plt.legend()
plt.savefig('./runs/tsne/17/')
