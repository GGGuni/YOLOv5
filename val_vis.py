...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader_wEDGE(data[task], imgsz, batch_size, gs, single_cls, pad=pad, rect=True,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            tmp_pred = None
            for idx in range(len(pred[:, 5])):
                if pred[:, 5][idx] == 0:
                    pred[:, 5][idx] = 0
                    if tmp_pred == None:
                        tmp_pred = pred[idx].reshape(-1,6)
                    else:
                        tmp_pred = torch.cat((tmp_pred, pred[idx].reshape(-1,6)))
        ...
        if plots:
            f = save_dir / f'test{batch_i}_labels.jpg'  # labels
            # plot_images(img, targets, paths, f, names)
            Thread(target=plot_images, args=(img, targets, paths, f, names), daemon=True).start()
            f = save_dir / f'test{batch_i}_pred.jpg'  # predictions
            Thread(target=plot_images, args=(img, output_to_target(out), paths, f, names), daemon=True).start()
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / './dronepic_label_size_tr_oc.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp53/weights/best.pt', help='model.pt path(s)')
