...
def get_new_indices(indices, cand):
    ...
    for i in range(len(indices)):
        for k in range(3):
            if cand[..., 9:10][:, k, indices[i, 0], indices[i, 1]] > 0.6:
...
def get_correct_indices(indices, cand, check_labels):
    ...
    for i in range(len(indices)):
            for k in range(3):
                index = np.argmax(cand[...,4:9][:, k, indices[i][0], indices[i][1]].cpu())
    ...
    return torch.tensor(f_indices).reshape(-1,2)
...
for img_path in img_list:
    ...
    for i in range(len(tbox_gt)):
        indices_1 = cal_IOU_wFeatureMap(new_cand_1, torch.tensor(tbox_gt[i]).cuda())
        new_index_1 = get_new_indices(indices_1, cand_1)
        if len(new_index_1)==0 :
            continue
        else:
            new_indices_1.append(new_index_1)


        for k in range(len(new_index_1)):
            check_labels_1.append(labels[i][0])

    # new_indices_1 = torch.tensor(new_indices_1).reshape(-1,2)
    new_indices_1 = torch.cat(new_indices_1, 0)

    final_indices_1 = get_correct_indices(new_indices_1, cand_1, check_labels_1)
    ...
    for i in range(len(tbox_gt)):
        indices_2 = cal_IOU_wFeatureMap(new_cand_2, torch.tensor(tbox_gt[i]).cuda())
        new_indices_2 = get_new_indices(indices_2, cand_2)
        for j in range(len(new_indices_2)):
            s_features_20.append(features[20][..., new_indices_2[j][0], new_indices_2[j][1]].cpu())
            s_labels_20.append(labels[i][0])

    for i in range(len(tbox_gt)):
        indices_3 = cal_IOU_wFeatureMap(new_cand_3, torch.tensor(tbox_gt[i]).cuda())
        new_indices_3 = get_new_indices(indices_3, cand_3)
        for j in range(len(new_indices_3)):
            s_features_23.append(features[23][..., new_indices_3[j][0], new_indices_3[j][1]].cpu())
...
def plot_image(tsne, s_labels, layer_num):
    ...
    plt.legend()
    plt.savefig('./runs/tsne/'+str(layer_num)+'/all_'+str(layer_num)+'23.png')
    plt.subplot(3, 2, 1)
    plt.scatter(np.array(xy_person)[:, 0], np.array(xy_person)[:, 1], label='person', s=0.5, edgecolors='none')
    plt.title('person')
    plt.subplot(3, 2, 2)
    ...
    plt.scatter(np.array(xy_bike)[:, 0], np.array(xy_bike)[:, 1], label='bike', s=0.5, c='mediumpurple',
                edgecolors='none')
    plt.title('bike')
    plt.savefig('./runs/tsne/'+str(layer_num)+'/23class.png')
